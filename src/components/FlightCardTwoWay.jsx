...
const FlightCardTwoWay = ({ className = "", data, setSelectedReturnFlight }) => {
  ...
  const renderDetail = () => {
    ...
    return (
      <div className="rounded-2xl border border-neutral-200 p-4 dark:border-neutral-700 md:p-8">
        ...
        {renderDetailTop()}
        <button onClick={()=>setSelectedReturnFlight(data)} className="bg-black px-4 py-2 w-fit rounded relative bottom-6 text-white float-right hover:bg-gray-800 duration-500 cursor-pointer">Select</button>
      </div>
    );
  };
